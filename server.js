...
function calculateAuction(strikePrice) {
  const { sharesPerParticipant, preAuctionPrice, buybackPool, priceMin, priceMax } = state.config;

  // Post-auction market price: adjusts based on where strike lands in the range
  const midPrice = (priceMin + priceMax) / 2;
  const sensitivity = 0.5;
  const postAuctionPrice = Math.round((preAuctionPrice + (strikePrice - midPrice) * sensitivity) * 100) / 100;
  ...
  for (const [pid, p] of Object.entries(state.participants)) {
    ...
    // Value of remaining shares at post-auction market price
    const remainingValue = sharesRemaining * postAuctionPrice;
